...
export default function Navbar() {
  ...
  const initials = getInitials(session?.user?.name ?? "Yibrah Mehari");

  return (
    <>
      <header className="bg-blue-600 fixed top-0 right-0 w-full left-0 h-16 flex items-center justify-between px-16 text-slate-50 z-50">
        <Link legacyBehavior href="/">
          <a className="font-bold text-2xl md:text-xl">
            Tigray Automotive
            <sup> IMS</sup>
          </a>
        </Link>
